...
public class DirectoryScannerTests
{
    ...
    [Fact]
    public async Task Scan_FindsExpectedFileCountsByFamily()
    {
        var photos = await ScanAsync();

        Assert.Equal(57, photos.Count);
        Assert.Equal(5, photos.Count(p => p.Family == FormatFamily.Jpeg));
        Assert.Equal(52, photos.Count(p => p.Family == FormatFamily.Raw));
        Assert.DoesNotContain(photos, p => p.Family == FormatFamily.Png);
    }
