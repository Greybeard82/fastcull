...
namespace Fastcull.ViewModels
{
    public partial class FilmstripItemViewModel : ObservableObject, ICacheableItem, IDisposable
    {
        ...
        /// </summary>
        public ImageSource? StageImageSource
        {
            get
            {
                // Logged from the GETTER on purpose: it records that the binding actually re-read
                // after a change, which is a different fact from the property having changed.
                Diagnostics.ZoomTrace.Log("get StageImageSource",
                    $"{Photo.FileName} zoom={(ZoomImage is null ? "null" : "SET")} display={(DisplayImage is null ? "null" : "SET")}");

                return ZoomImage ?? DisplayImage;
            }
        }
        ...
        /// <summary>
        /// The gated form of <see cref="IsZoomLoading"/>, and what the UI actually binds to.
        ///
        /// **Why the raw flag is not bound directly.** Measured, the zoom decode takes 146-229 ms
        /// on this hardware for both RAW and a 9000x6000 PNG, and the size-change supersede toggles
        /// the flag off and on again inside that window. Binding straight through put a spinner on
        /// screen for about a sixth of a second, flickering once - which reads as "the loading
        /// indicator never appears", the reported symptom, even though every link in the chain was
        /// working. A screenshot taken during a deliberately stalled decode showed the element
        /// rendering perfectly; the only thing wrong with it was how long it lasted.
        ///
        /// So: do not show it for the first <see cref="ShowDelayMs"/>, and once shown keep it for
        /// at least <see cref="MinimumShowMs"/>. A fast decode now shows nothing at all rather than
        /// a flash, and a slow one - a 400% magnified re-decode, a slower machine - shows something
        /// a person can actually read.
        /// </summary>
        private bool _showZoomLoading;

        /// <summary>Below this, a decode is quick enough that an indicator is just a flash.</summary>
        private const int ShowDelayMs = 180;

        /// <summary>Once shown, stay up this long so it can be read rather than glimpsed.</summary>
        private const int MinimumShowMs = 450;

        private Microsoft.UI.Dispatching.DispatcherQueueTimer? _loadingTimer;
        ...
        public Visibility ZoomLoadingVisibility
        {
            get
            {
                var visible = _showZoomLoading && IsZoomed;

                Diagnostics.ZoomTrace.Log("get ZoomLoadingVisibility",
                    $"{Photo.FileName} show={_showZoomLoading} loading={IsZoomLoading} zoomed={IsZoomed} -> {(visible ? "Visible" : "Collapsed")}");

                return visible ? Visibility.Visible : Visibility.Collapsed;
            }
        }

        /// <summary>Drives the ProgressRing, so it only spins while the pill is actually up.</summary>
        public bool IsZoomLoadingShown => _showZoomLoading;

        private void SetShowZoomLoading(bool value)
        {
            if (_showZoomLoading == value) return;

            _showZoomLoading = value;
            if (value) _loadingShownAt = DateTime.UtcNow;

            Diagnostics.ZoomTrace.Log("SHOW indicator", $"{Photo.FileName} -> {value}");

            OnPropertyChanged(nameof(IsZoomLoadingShown));
            OnPropertyChanged(nameof(ZoomLoadingVisibility));
        }
        ...
        private Microsoft.UI.Dispatching.DispatcherQueueTimer Timer()
        {
            if (_loadingTimer is null)
            {
                ...
                _loadingTimer.Tick += (_, _) => OnLoadingTimerTick();
            }
        ...
        private void OnLoadingTimerTick()
        {
            if (IsZoomLoading)
            {
                // The delay elapsed and the decode is still running: it is slow enough to be worth
                // reporting.
                SetShowZoomLoading(true);
                return;
            }

            // The minimum display time elapsed and the decode has since finished.
            SetShowZoomLoading(false);
        }

        /// <summary>
        /// Generated by [ObservableProperty]. Records that the setter genuinely changed value - a
        /// setter that is called with the value it already holds raises no notification and is
        /// indistinguishable from one that was never called at all.
        /// </summary>
        partial void OnIsZoomLoadingChanged(bool value)
        {
            Diagnostics.ZoomTrace.Log("SET IsZoomLoading", $"{Photo.FileName} -> {value}");
            ...
            if (value)
            {
                // Not shown yet: wait and see whether this decode is slow enough to be worth
                // mentioning. Already shown (a supersede restarting the decode): leave it up, so
                // the off-and-on at the size change is invisible rather than a flicker.
                if (_showZoomLoading) return;

                timer.Interval = TimeSpan.FromMilliseconds(ShowDelayMs);
                timer.Start();
                return;
            }

            if (!_showZoomLoading) return;   // never appeared; nothing to take down

            var shownFor = (DateTime.UtcNow - _loadingShownAt).TotalMilliseconds;
            var remaining = MinimumShowMs - shownFor;
            ...
            timer.Interval = TimeSpan.FromMilliseconds(remaining);
            timer.Start();
        }

        partial void OnZoomImageChanged(ImageSource? value)
            => Diagnostics.ZoomTrace.Log("SET ZoomImage", $"{Photo.FileName} -> {(value is null ? "null" : "SoftwareBitmapSource")}");
        ...
        public async Task LoadZoomImageAsync(uint longEdge)
        {
            ...
            Diagnostics.ZoomTrace.Log("ENTER LoadZoomImage",
                $"{Photo.FileName} longEdge={longEdge} family={Photo.Family} isZoomed={IsZoomed}");
            ...
            try
            {
                ...
                await Diagnostics.ZoomTrace.StallAsync(cts.Token);

                var bitmap = await DecodeZoomTierAsync(longEdge, cts.Token);

                Diagnostics.ZoomTrace.Log("decode returned",
                    $"{Photo.FileName} bitmap={(bitmap is null ? "NULL" : $"{bitmap.PixelWidth}x{bitmap.PixelHeight}")} "
                    + $"cancelled={cts.IsCancellationRequested} stillCurrent={ReferenceEquals(_zoomCts, cts)}");
                ...
                Diagnostics.ZoomTrace.Log("SetBitmapAsync done",
                    $"{Photo.FileName} cancelled={cts.IsCancellationRequested} stillCurrent={ReferenceEquals(_zoomCts, cts)}");
            ...
            {
                Diagnostics.ZoomTrace.Log("EXIT LoadZoomImage",
                    $"{Photo.FileName} stillCurrent={ReferenceEquals(_zoomCts, cts)} loadingWas={IsZoomLoading}");
        ...
        {
            Diagnostics.ZoomTrace.Log("CancelZoomDecode",
                $"{Photo.FileName} hadCts={_zoomCts is not null} loading={IsZoomLoading}");
